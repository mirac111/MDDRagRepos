...
async def completion(tenant_id, agent_id, session_id=None, **kwargs):
    ...
    try:
        if session_id:
            ...
            logging.info(f"[DEBUG] session_id={session_id} e={e}")
            if not e:
                raise LookupError("Session not found!")
            if not conv.message:
                conv.message = []
            logging.info(f"[DEBUG] conv.dsl type={type(conv.dsl)} start={str(conv.dsl)[:30] if conv.dsl else 'NONE'}")
            if not isinstance(conv.dsl, str):
                conv.dsl = json.dumps(conv.dsl, ensure_ascii=False)
            logging.info(f"[DEBUG] Creating Canvas...")
            canvas = Canvas(conv.dsl, tenant_id, agent_id, canvas_id=agent_id, custom_header=custom_header)
            logging.info(f"[DEBUG] Canvas OK")
        else:
        ...
        logging.info(f"[DEBUG] Starting canvas.run()...")
        async for ans in canvas.run(**run_kwargs):
            if ans is None:
                logging.error(f"[DEBUG] ans is None! Skipping.")
                continue
        ...
        logging.info(f"[DEBUG] canvas.run() finished")
        ...
        logging.exception(f"[DEBUG] EXCEPTION in completion: {type(ex).__name__}: {ex}")
        raise
